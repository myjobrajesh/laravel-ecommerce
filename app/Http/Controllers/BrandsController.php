...
class BrandsController extends Controller {
    ...
    public function store(BrandsRequest $request) {
        ...
        /*if (Auth::user()->id == 2) {
            ...
            \CommonHelper::flash()->error('Error', 'Cannot create Brand because you are signed in as a test user.');
        } else {
          */
            ...
            \CommonHelper::flash()->success('Success', 'Brand created successfully!');
        //}

        // Redirect back to Show all brands page.
        return redirect('admincare/shop/brands');
    }
    ...
    public function update($id, BrandsRequest $request) {
        ...
        if (Auth::user()->id == 2) {
            // If user is a test user (id = 2),display message saying you cant delete if your a test user
            \CommonHelper::flash()->error('Error', 'Cannot edit Brand because you are signed in as a test user.');
        } else {
            // Update the brand with Request rules
            $brands->update($request->all());

            // Flash success message
            \CommonHelper::flash()->success('Success', 'Brand update successfully!');
        }
    ...
    public function delete($id) {
        ...
        if ($products > 0) {
            ...
            \CommonHelper::flash()->customErrorOverlay('Error', 'There are products under this brand. Cannot delete this brand until all products under this brand are deleted');
        }/* elseif (Auth::user()->id == 2) {
            // If user is a test user (id = 2),display message saying you cant delete if your a test user
            \CommonHelper::flash()->error('Error', 'Cannot delete Brand because you are signed in as a test user.');
        } */else {
            $delete->delete();
        }
