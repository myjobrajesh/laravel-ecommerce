...
@section('content')
    ...
    <div class="container-fluid" id="admin-product-container">
        ...
        <a href="{{ url('admincare/shop/product/add') }}" class="btn btn-primary">Add new Product</a>
        <br><br>
        ...
        <table class="table table-bordered table-condensed table-hover">
            ...
            <tr>
                <td class="text-center">
                    <form method="post" action="{{ route('admin.shop.product.delete', $products->id) }}" class="delete_form_product">
                        {{ csrf_field() }}
                ...
                <td class="text-center">
                    <a href="{{ route('admin.shop.product.edit', $products->id) }}">
                        <i class="material-icons green-text">mode_edit</i>
                    </a>
                </td>
                <td class="text-center">
                    <a href="{{ URL('/admincare/shop/products', $products->id) }}">
                ...
                <td class="text-center" style="width: 20%;">
                    ...
                    @else
                        @if ($products->featuredPhoto)
                            <img src="{{ \ImageHelper::viewFile($products->featuredPhoto->filepath, 'sm') }}" alt="Photo ID: {{ $products->featuredPhoto->id }}" />
                        @elseif(!$products->featuredPhoto)
                            <img src="{{ \ImageHelper::viewFile($products->photos->first()->filepath, 'sm')}}" alt="Photo" />
                        @else
                            N/A
                        @endif
                    @endif
